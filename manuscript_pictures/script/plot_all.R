...
library(pheatmap)
library(BuenColors)
setwd('C:/Users/Administrator/Desktop/chenkailin2')
###########################
### larry
###########################
### pheatmap
larry_cluster <- read.csv("C:/Users/Administrator/Desktop/chenkailin2/重编程（只测prog）/simple_matrix.csv",
                          header=FALSE)
larry_cluster[larry_cluster>0.0006]=0.0006
larry_cluster[larry_cluster<0.0002]=0.0002
colnames(larry_cluster) = paste0('Cluster',1:12)
rownames(larry_cluster) = paste0('Cluster',1:12)
...
dev.off()
### acc col plot
df_acc = data.frame('accuracy'=c(0.8876,0.69,0.61),
                    'methods'=c('KAILIN',
                                'cospar_state_info',
                                'Waddington-OT'))
